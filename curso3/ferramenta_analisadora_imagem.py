...
class FerramentaAnalisadorImagem(BaseTool):
    ...
    description: str = """
    Utilize esta ferramenta sempre que for solicidade que você fala uma análise
    de imagem.
        ...
        template_analisador = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """
                    Assuma que voce é um analisador de imagens. A sua tarefa principal
                    consiste em: analisar uma imagem e extrair informações importantes
                    de form objetiva.

                    # FORMATO DE SAÍDA
                    Descrição de Imagem: 'Coloque a sua descrição da imagem aqui'
                    Rótulos: 'Coloque uma lista com três temos chave separados por virgula'
                    """
                ),
                (
                    "user",
                    [
                        ...
                        },
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": "data:image/jpeg;base64,{imagem_informada}",
                            }
                        }
                    ]
                ),
